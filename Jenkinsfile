          ...
          retry(5) {
            sh '''#! /bin/bash
                  ...
                  for MANIFESTIMAGE in "${IMAGE}" "${GITLABIMAGE}" "${GITHUBIMAGE}" "${QUAYIMAGE}"; do
                    ...
                    docker manifest create ${MANIFESTIMAGE}:edge ${MANIFESTIMAGE}:amd64-edge ${MANIFESTIMAGE}:arm64v8-edge
                    ...
                    digest=$(curl -s \
                      ...
                      "https://ghcr.io/v2/linuxserver/${CONTAINER_NAME}/manifests/arm32v7-edge")
                    if [[ $(echo "$digest" | jq -r '.layers') != "null" ]]; then
                      docker manifest push --purge ${MANIFESTIMAGE}:arm32v7-edge || :
                      docker manifest create ${MANIFESTIMAGE}:arm32v7-edge ${MANIFESTIMAGE}:amd64-edge
                      docker manifest push --purge ${MANIFESTIMAGE}:arm32v7-edge
                    fi
                    docker manifest push --purge ${MANIFESTIMAGE}:edge
                    docker manifest push --purge ${MANIFESTIMAGE}:${META_TAG} 
